...
module CPU(
    ...
    // For BP...
    // 말짱할 때와, 1 -> brabch, 2 -> Jump !
    assign update_enable = id_valid_inst && !id_halt && !stall_active &&
                       (id_PCSource == 2'd1 || id_PCSource == 2'd2);
    // default 값이 필요해서, 그냥 그럭저럭 현재 PC 값 넣어주기...
    // '32b0 넣어주고 싶긴한데, 그럼 뭔가 더 큰 오류가 있을 듯... enable이 반드시 걸러주긵할듯
    // 만일 들어간다고해도 프로그램 자체에 영향은 없고 성능만 좀 느려지도록 말짱한 PC 값을 일단
    assign update_target = (id_PCSource == 2'd2) ? id_jump_target :
                            branch_taken ? id_branch_target : IFID_PC;

    always @(*) begin
        // Forwarding Logic... 사실 ppt와 완전 동일하다
        // rs_EX 가 없긴하다... 
        if (EXMEM_RegWrite && (EXMEM_RegAddr == IDEX_RS)) ForwardA = 2'b01;
        else if (MEMWB_RegWrite && (MEMWB_RegAddr == IDEX_RS)) ForwardA = 2'b10;
        else ForwardA = 2'b00;
    
        if (EXMEM_RegWrite && (EXMEM_RegAddr == IDEX_RT)) ForwardB = 2'b01;
        else if (MEMWB_RegWrite && (MEMWB_RegAddr == IDEX_RT)) ForwardB = 2'b10;
        else ForwardB = 2'b00;
        ...
        // -> Ex stage signal...
        ext_imm = IDEX_SignExtend ? {{16{IDEX_IMMI[15]}}, IDEX_IMMI} : {16'b0, IDEX_IMMI};
        ...
        operand2 = IDEX_ALUSrc ? ext_imm : operand2_fwd;
    end
